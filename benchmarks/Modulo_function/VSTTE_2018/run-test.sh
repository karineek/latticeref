...
function test_one {
  ...
  echo ">> Run test case: ulimit -Sv 3000000; ulimit -St 500; $hifrog $1 --logic $2 --save-summaries ${SUMMARIES}" $p4 $p5 $p6 $p7 $p8 $p9 $p10 $p11 $p12 $p13 $p14 $p15
  ulimit -Sv 3000000; ulimit -St 500; $hifrog $1 --logic $2 --save-summaries ${SUMMARIES} $p4 $p5 $p6 $p7 $p8 $p9 $p10 $p11 $p12 $p13 $p14 $p15 $p16 > ${HIFROG_OUTPUT} 2>&1
  if [[ $? -gt 0 ]]; then
...
function test_cbmc {
  ...
  PREFIX=$PATH_reg$OUTDIR/${INPUT: : -2}
  FILE_PREFIX="${PREFIX}/${OUTPUT_PREFIX}_$2"
  HIFROG_OUTPUT="${FILE_PREFIX}_cbmc_$IND.txt"
  HIFROG_RESULT="${FILE_PREFIX}_result.txt"
  HIFROG_TIME="${FILE_PREFIX}_time.txt"
  EXPECTED_OUTPUT="${INPUT}_out"
  EXPECTED_OUTPUT_CUF="${FILE_PREFIX}_cuf_expected_res.txt"
  ...
  #stupid way to do it, but it works. If needed add more params
  echo ">> Run test case: ulimit -Sv 3000000; ulimit -St 500; $cbmc $1 --unwind 10"
  ulimit -Sv 3000000; ulimit -St 500; $cbmc $1 --unwind 10 >> ${HIFROG_OUTPUT} 2>&1
  if [[ $? -gt 0 ]]; then
    echo "CBMC analysis failed (see ${HIFROG_OUTPUT})"
    echo "Verify output against: $EXPECTED_OUTPUT"
  fi
  ...
  ## Refine option
  #stupid way to do it, but it works. If needed add more params
  echo ">> Run test case: ulimit -Sv 3000000; ulimit -St 500; $cbmc $1 --unwind 10 --refine"
  ulimit -Sv 3000000; ulimit -St 500; $cbmc $1 --unwind 10 --refine >> ${HIFROG_OUTPUT} 2>&1
  if [[ $? -gt 0 ]]; then
    echo "CBMC analysis failed (see ${HIFROG_OUTPUT})"
    echo "Verify output against: $EXPECTED_OUTPUT"
  fi
...
hifrog=./../hifrog/trunk/cprover/build/hifrog
cbmc=./../hifrog/trunk/cprover/build/bin/cbmc
...
do
	...
	arr=(`echo $temp | sed 's/,/\n/g'`)
		...
		mkdir -p "$OUTDIR/${arr[1]: : -2}" 
                echo "***** Create Directory for test-case ${arr[1]} : SKIP *****"
                #test_cbmc ${arr[1]} "cbmc" ${arr[0]} ${arr[3]} ${arr[4]}
        fi
