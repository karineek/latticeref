...
function test_one {
  ...
  if [ "$2" == "$QFCUF" ] ; then
    echo "Test ignored skipping CUF."
    ((IGNORED++))
    return 1
  fi
  ...
  ulimit -Sv 3000000; ulimit -St 1000; $hifrog "$1" --logic "$2" --save-summaries ${SUMMARIES} $p4 $p5 $p6 $p7 $p8 $p9 $p10 $p11 $p12 $p13 $p14 $p15 $p16 >> ${HIFROG_OUTPUT} 2>&1
  if [[ $? -gt 0 ]]; then
